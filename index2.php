...
$dir='down';
while (true) {
    echo 'THE CURRENT ATTACK POWER = ' . $attackPower . PHP_EOL;
    //sleep(1);
    $input = $initialInput;
    $units = $initialUnits;
    foreach ($units as $unit) {
        echo '[' . implode(', ', $unit) . ']' . PHP_EOL;
    }
    stamp($input);
    $round = 0;
    while (true) {
        ...
        for ($u = 0; $u < count($units); ++$u) {
            ...
            $unit = $units[$u];
            echo 'current [' . implode(', ', $unit) . ']' . PHP_EOL;
            $targets = [];
            ...
            if (count($targets) > 0) {
                ...
                $units[$bestTarget][2] -= $unit[1] === 'E' ? $attackPower : 3;
        ...
        if ($remaining['E'] === 0 || $remaining['G'] === 0) {
            ...
            foreach ($remaining as $species => $score) {
                echo $species . ', points: ' . $score . ', outcome: ' . ($score * $round) . PHP_EOL; 
            }
            break;
        }
        $round++;
    }
    if ($survivedElves === $totalElves) {
        if ($dir == 'down') {
            $attackPower = $attackPower - max([floor($attackPower / 2), 1]);
        } else {
            echo 'FOUND!' . PHP_EOL;
            echo 'attack power = ' . $attackPower . PHP_EOL;
            break;
        }
    } else {
        $dir = 'up';
